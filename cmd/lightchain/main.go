...
import (
	...
	"github.com/lightstreams-network/lightchain/blockchain/transaction"
	bcApp "github.com/lightstreams-network/lightchain/blockchain"
)
...
func LightchainNodeCmd(ctx *cli.Context) {
	...
	appNode := bcApp.CreateNode(tendermintLAddr, ctx)
	bcApp.StartNode(ctx, appNode)

	// Setup the ABCI server and start it
	addr := ctx.GlobalString(ABCIAddrFlag.Name)
	abci := ctx.GlobalString(ABCIProtocolFlag.Name)

	// Fetch the registered service of this type
	var ethBackend *ethereum.Backend
	if err := appNode.Service(&ethBackend); err != nil {
	...
	ethApp, err := bcApp.CreateLightchainApplication(ethBackend, rpcClient, nil, txManager)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	...
	appSrv, err := tmtServer.NewServer(addr, abci, ethApp)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
...
func InitCmd(ctx *cli.Context) error {
	if err := bcApp.InitNode(ctx); err != nil {
		return err
	}
